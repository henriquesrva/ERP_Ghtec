...
async function createNewUser(data) {
  ...
  const id = await repo.createUser({ nome, username, password_hash, role });
...
async function changePassword(userId, currentPassword, newPassword) {
  ...
  const full = await repo.findUserByUsername(user.username);
...
async function changeUserRole(targetId, newRole, requestingUserId) {
  ...
  if (target.role === "admin" && newRole !== "admin" && await repo.countAdmins() <= 1) {
    throw { status: 400, message: "Não é possível remover ou alterar o último administrador do sistema." };
  }
  await repo.updateUserRole(targetId, newRole);
}
...
async function deleteUser(id, requestingUserId) {
  ...
  if (target && target.role === "admin" && await repo.countAdmins() <= 1) {
    throw { status: 400, message: "Não é possível remover ou alterar o último administrador do sistema." };
  }
...
async function updateSignature(userId, { cargo, telefone }) {
  const user = await repo.findUserById(userId);
  if (!user) throw { status: 404, message: "Usuário não encontrado." };
  await repo.updateUserSignature(userId, {
